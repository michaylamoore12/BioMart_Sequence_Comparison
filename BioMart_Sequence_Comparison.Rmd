...
# Retrieve the amino acid sequences for BMP9 and BMP10
genes <- c("GDF2", "BMP10")
...
bmp9_seq <- AAString(sequences[sequences$hgnc_symbol == "GDF2", "peptide"])
bmp10_seq <- AAString(sequences[sequences$hgnc_symbol == "BMP10", "peptide"])
...
cat("Sequence Identity between BMP9 and BMP10:", identity, "%\n")
```
...
print("Human GDF2 sequence:")
print(human_sequences)
print("Mouse GDF2 sequence:")
...
if (length(human_sequences$peptide) > 0 & length(mouse_sequences$peptide) > 0) {
  ...
  print("Pairwise alignment between human and mouse GDF2:")
  print(alignment)
  
  # Calculate the sequence identity
  identity <- pid(alignment, type = "PID3")  # Percentage Identity based on alignment
  cat("Sequence Identity between Human and Mouse GDF2:", identity, "%\n")
  
} else {
  cat("Could not retrieve the sequences for GDF2 from Ensembl.\n")
}
